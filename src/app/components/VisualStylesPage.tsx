 ...
 *
 * Datos de marca en `src/app/data/visualStyles.ts`; reporte de completitud
 * en `src/app/lib/siteCompleteness.ts` (`visualStylesReports`). Los tamaños
 * de ícono (`ICON_SIZES`) son una escala técnica fija, no una decisión de
 * marca por ítem — quedan estructurales acá, no pasan por el reporte.
 *
...
function ShadowCard({ name, token, spec, shadow, usage }: { name: string; token: string; spec: string; shadow: string; usage: string }) {
  return (
    <div className="flex w-full min-w-0 flex-col gap-[12px] overflow-clip rounded-[16px] border border-[#b9c3ce] bg-white p-[20px]">
      ...
      <p className="font-bold text-[20px] leading-[26px] text-[#292e38]">{name}</p>
      <TokenTag fit>{token}</TokenTag>
      <p className="font-mono text-[12px] leading-[16px] text-[#576175]">{spec}</p>
      <p className="font-normal text-[13px] leading-[20px] text-[#576175]">{usage}</p>
    </div>
  )
}

/* ─── Sizing (íconos — escala técnica fija, no dato de marca) ─── */
...
export function VisualStylesPage() {
  const { spacing, radius, borders, shadows, componentHeights } = visualStylesReports
      ...
      <div className="flex w-full flex-col gap-[44px] px-[40px] py-[72px]">
        ...
        {shadows.visible.length > 0 && (
          <section className="flex w-full flex-col gap-[24px]">
            ...
            <div className="grid w-full grid-cols-1 gap-[16px] min-[900px]:grid-cols-2 min-[1600px]:grid-cols-3">
              {shadows.visible.map((s) => (
                <ShadowCard key={s.token} name={s.name!} token={s.token!} spec={s.spec!} shadow={s.shadow!} usage={s.usage!} />
              ))}
            </div>
          </section>
        )}
        ...
        {hasSizing && (
          <section className="flex w-full flex-col gap-[24px]">
            ...
            {componentHeights.visible.length > 0 && (
              <SizingCardShell
